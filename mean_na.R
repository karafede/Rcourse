...
O3_daily_8hourly <- function(EAD_data, year ){
...
# select by stations to loop 
  ...
  daily_O3_max<-gather( dawit_da_max, "Site" , "MAX_8hour", -date_gr )
  
  daily_data$Date <-dawit_da$date_gr
  
  daily_O3<-gather(daily_data, "Site" , "Mean_8hour" , -Date)
  daily_O3_cap<-gather(daily_capture, "Site" , "Capture" )
  
  daily_O3$MAX_8hour<-daily_O3_max$MAX_8hour
  daily_O3$Capture<-daily_O3_cap$Capture
  
  
  # results of the function O3_daily_8hourly
  result<- list(average_O3_all,daily_O3)
  return(result)
